...
using aractalep.Authentication;
using aractalep.Entities;
using aractalep.EntityFrameworkCore;

namespace aractalep.Web.Host.Controllers
{
        ...
        public ActionResult Authenticate([FromBody] AuthenticateModel model)
        {
            try
            {
                ...
                if (user.PasswordHash != null && user.PasswordHash.StartsWith("PBKDF2$"))
                    pwOk = aractalep.Users.PasswordHelper.VerifyPbkdf2(model.Password, user.PasswordHash);
                else

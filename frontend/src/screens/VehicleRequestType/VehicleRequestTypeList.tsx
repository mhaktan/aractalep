...
import { VehicleRequestTypeCreate } from './VehicleRequestTypeCreate';
import { VehicleRequestTypeEdit } from './VehicleRequestTypeEdit';
...
// ---------------------------------------------------------------------------
// VehicleRequestTypeList
// ---------------------------------------------------------------------------

export const VehicleRequestTypeList: React.FC = () => {
  const list = useListQuery<VehicleRequestTypeRecord>({ resource: 'VehicleRequestType' });
  const [showCreate, setShowCreate] = useState(false);
  const [editRecord, setEditRecord] = useState<VehicleRequestTypeRecord | null>(null);
  const [selectedRows, setSelectedRows] = useState<VehicleRequestTypeRecord[]>([]);
  ...
  useEffect(() => {
    ...
    Promise.resolve(dataProvider.getOne('VehicleRequestType', focusId))
      .then((rec) => { if (rec) setEditRecord(rec as VehicleRequestTypeRecord); })
      .catch(() => { /* kayit bulunamadi — liste normal acilir */ })
      .finally(clear);
  }, [focusId]);
  const { triggerFlows } = useFlows();
  const del = useDeleteMutation('VehicleRequestType', (ids) => { ids.forEach(id => triggerFlows('delete', 'VehicleRequestType', { id })); });
  ...
  return (
    <>
      <ListPageLayout
        ...
        onSelectionChange={(rows) => setSelectedRows(rows as VehicleRequestTypeRecord[])}
        ...
        headerActions={<>
          ...
          <TkButton label="+ Create VehicleRequestType" variant="primary" onTkClick={() => setShowCreate(true)} />
        </>}
      />
      ...
      />
      <VehicleRequestTypeCreate open={showCreate} onClose={() => setShowCreate(false)} onSuccess={list.invalidate} />
      <VehicleRequestTypeEdit record={editRecord} onClose={() => setEditRecord(null)} onSuccess={list.invalidate} />

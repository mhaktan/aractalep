...
type VehicleRequestTypeRecord = {
  id: string | number;
  name: string;
  description?: string;
};

interface VehicleRequestTypeEditProps {
  record: VehicleRequestTypeRecord | null;
  onClose: () => void;
  onSuccess: () => void;
}

export const VehicleRequestTypeEdit: React.FC<VehicleRequestTypeEditProps> = ({ record, onClose, onSuccess }) => {
  const [form, setForm] = useState<Partial<VehicleRequestTypeRecord>>(record ?? {});
  ...
  const mutation = useMutation({
    mutationFn: (values: Partial<VehicleRequestTypeRecord>) =>
      dataProvider.update('VehicleRequestType', record!.id, values),
    onSuccess: (_data, values) => { triggerFlows('update', 'VehicleRequestType', values as Record<string, unknown>); onSuccess(); onClose(); },
    ...
    <div style={overlayStyle} onClick={onClose}>
      <div style={modalStyle} onClick={(e) => e.stopPropagation()}>
        <div style={{ padding: '20px 28px 0', flexShrink: 0, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          <h2 style={{ margin: 0, fontSize: 18, fontWeight: 700 }}>Edit VehicleRequestType</h2>

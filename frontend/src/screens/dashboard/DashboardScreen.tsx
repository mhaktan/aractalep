...
export const DashboardScreen: React.FC = () => {
  ...
  const [total_departmentData, setTotal_departmentData] = React.useState<Record<string, unknown> | null>(null);
  const [total_departmentLoading, setTotal_departmentLoading] = React.useState(true);
  React.useEffect(() => {
    const fetchData = async () => {
      try {
        const res = await fetch(`${API_BASE}/api/services/app/Department/GetAll?MaxResultCount=1`, { headers: getRequestHeaders() });
        ...
        setTotal_departmentData(json);
      } catch { /* ignore */ }
      finally { setTotal_departmentLoading(false); }
    };
    fetchData();
  }, []);

  const [total_vehiclerequesttypeData, setTotal_vehiclerequesttypeData] = React.useState<Record<string, unknown> | null>(null);
  const [total_vehiclerequesttypeLoading, setTotal_vehiclerequesttypeLoading] = React.useState(true);
  React.useEffect(() => {
    const fetchData = async () => {
      try {
        const res = await fetch(`${API_BASE}/api/services/app/VehicleRequestType/GetAll?MaxResultCount=1`, { headers: getRequestHeaders() });
        // Token expired or invalid — clear auth and redirect to login
        if (res.status === 401) {
          ['_auth_token', '_bearer_token', '_refresh_token'].forEach(k => localStorage.removeItem(k));
          if (window.location.pathname !== '/login') window.location.href = '/login';
        ...
        // Auto-unwrap ABP/generic envelopes so {result.totalCount} or bare {totalCount} both work
        const json = unwrapResponse(rawJson) as Record<string, unknown>;
        setTotal_vehiclerequesttypeData(json);
      } catch { /* ignore */ }
      finally { setTotal_vehiclerequesttypeLoading(false); }
    };
    fetchData();
  }, []);

  const [total_vehicleData, setTotal_vehicleData] = React.useState<Record<string, unknown> | null>(null);
  const [total_vehicleLoading, setTotal_vehicleLoading] = React.useState(true);
  React.useEffect(() => {
    const fetchData = async () => {
      try {
        const res = await fetch(`${API_BASE}/api/services/app/Vehicle/GetAll?MaxResultCount=1`, { headers: getRequestHeaders() });
        // Token expired or invalid — clear auth and redirect to login
        if (res.status === 401) {
          ['_auth_token', '_bearer_token', '_refresh_token'].forEach(k => localStorage.removeItem(k));
          if (window.location.pathname !== '/login') window.location.href = '/login';
        ...
        // Auto-unwrap ABP/generic envelopes so {result.totalCount} or bare {totalCount} both work
        const json = unwrapResponse(rawJson) as Record<string, unknown>;
        setTotal_vehicleData(json);
      } catch { /* ignore */ }
      finally { setTotal_vehicleLoading(false); }
    };
    fetchData();
  }, []);

  const [total_vehiclerequestData, setTotal_vehiclerequestData] = React.useState<Record<string, unknown> | null>(null);
  const [total_vehiclerequestLoading, setTotal_vehiclerequestLoading] = React.useState(true);
  React.useEffect(() => {
    const fetchData = async () => {
      try {
        const res = await fetch(`${API_BASE}/api/services/app/VehicleRequest/GetAll?MaxResultCount=1`, { headers: getRequestHeaders() });
        // Token expired or invalid — clear auth and redirect to login
        if (res.status === 401) {
          ['_auth_token', '_bearer_token', '_refresh_token'].forEach(k => localStorage.removeItem(k));
          if (window.location.pathname !== '/login') window.location.href = '/login';
        ...
        // Auto-unwrap ABP/generic envelopes so {result.totalCount} or bare {totalCount} both work
        const json = unwrapResponse(rawJson) as Record<string, unknown>;
        setTotal_vehiclerequestData(json);
      } catch { /* ignore */ }
      finally { setTotal_vehiclerequestLoading(false); }
    };
    fetchData();
  }, []);
  ...
  React.useEffect(() => {
    const sources = [
      { name: 'Birim', url: `${API_BASE}/api/services/app/Department/GetAll?MaxResultCount=1`, path: 'result.totalCount' },
      { name: 'Talep Türü', url: `${API_BASE}/api/services/app/VehicleRequestType/GetAll?MaxResultCount=1`, path: 'result.totalCount' },
      { name: 'Araç', url: `${API_BASE}/api/services/app/Vehicle/GetAll?MaxResultCount=1`, path: 'result.totalCount' },
      { name: 'Araç Talebi', url: `${API_BASE}/api/services/app/VehicleRequest/GetAll?MaxResultCount=1`, path: 'result.totalCount' },
  ...
  const [recent_department_tableData, setRecent_department_tableData] = React.useState<Record<string, unknown> | null>(null);
  const [recent_department_tableLoading, setRecent_department_tableLoading] = React.useState(true);
  React.useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        setRecent_department_tableData(target != null ? (target as Record<string, unknown>) : json);
      } catch { /* ignore */ }
      finally { setRecent_department_tableLoading(false); }
    };
    fetchData();
  }, []);

  const [recent_vehiclerequesttype_tableData, setRecent_vehiclerequesttype_tableData] = React.useState<Record<string, unknown> | null>(null);
  const [recent_vehiclerequesttype_tableLoading, setRecent_vehiclerequesttype_tableLoading] = React.useState(true);
  React.useEffect(() => {
    const fetchData = async () => {
      try {
        const res = await fetch(`${API_BASE}/api/services/app/VehicleRequestType/GetAll?MaxResultCount=5&Sorting=id desc`, { headers: getRequestHeaders() });
        // Token expired or invalid — clear auth and redirect to login
        if (res.status === 401) {
          ['_auth_token', '_bearer_token', '_refresh_token'].forEach(k => localStorage.removeItem(k));
          if (window.location.pathname !== '/login') window.location.href = '/login';
        ...
        const json = unwrapResponse(rawJson) as Record<string, unknown>;
        const target = getNestedValue(rawJson as Record<string, unknown>, 'result.items') ?? getNestedValue(json, 'result.items');
        setRecent_vehiclerequesttype_tableData(target != null ? (target as Record<string, unknown>) : json);
      } catch { /* ignore */ }
      finally { setRecent_vehiclerequesttype_tableLoading(false); }
    };
    fetchData();
  }, []);
  ...
  return (
    <div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(12, 1fr)', gap: 16 }}>
        <div style={{ gridColumn: 'span 12' }}>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 16 }}>
            <div style={{ gridColumn: 'span 2' }}>
              ...
              <UiCard bodyStyle={{ padding: 20 }}>
                <div style={{ fontSize: 12, color: '#888', marginBottom: 4 }}>Total Department</div>
                <div style={{ fontSize: 28, fontWeight: 700, color: '#1976d2' }}>{(getNestedValue(total_departmentData ?? {}, 'result.totalCount') as string | number) ?? '—'}</div>
              </UiCard>
              )}

            </div>
            <div style={{ gridColumn: 'span 1' }}>
              {total_vehiclerequesttypeLoading ? (
                <UiCard bodyStyle={{ padding: 20 }}>
                  <div style={{ height: 12, width: '40%', background: '#e0e0e0', borderRadius: 4, marginBottom: 12, animation: 'pulse 1.5s ease-in-out infinite' }} />
                  <div style={{ height: 28, width: '60%', background: '#e0e0e0', borderRadius: 4, marginBottom: 8, animation: 'pulse 1.5s ease-in-out infinite' }} />
                  <div style={{ height: 10, width: '50%', background: '#f0f0f0', borderRadius: 4, animation: 'pulse 1.5s ease-in-out infinite' }} />
              ...
              <UiCard bodyStyle={{ padding: 20 }}>
                <div style={{ fontSize: 12, color: '#888', marginBottom: 4 }}>Total VehicleRequestType</div>
                <div style={{ fontSize: 28, fontWeight: 700, color: '#4caf50' }}>{(getNestedValue(total_vehiclerequesttypeData ?? {}, 'result.totalCount') as string | number) ?? '—'}</div>
              </UiCard>
              )}

            </div>
            <div style={{ gridColumn: 'span 4' }}>
              {total_vehicleLoading ? (
                <UiCard bodyStyle={{ padding: 20 }}>
                  <div style={{ height: 12, width: '40%', background: '#e0e0e0', borderRadius: 4, marginBottom: 12, animation: 'pulse 1.5s ease-in-out infinite' }} />
                  <div style={{ height: 28, width: '60%', background: '#e0e0e0', borderRadius: 4, marginBottom: 8, animation: 'pulse 1.5s ease-in-out infinite' }} />
                  <div style={{ height: 10, width: '50%', background: '#f0f0f0', borderRadius: 4, animation: 'pulse 1.5s ease-in-out infinite' }} />
              ...
              <UiCard bodyStyle={{ padding: 20 }}>
                <div style={{ fontSize: 12, color: '#888', marginBottom: 4 }}>Total Vehicle</div>
                <div style={{ fontSize: 28, fontWeight: 700, color: '#ff9800' }}>{(getNestedValue(total_vehicleData ?? {}, 'result.totalCount') as string | number) ?? '—'}</div>
              </UiCard>
              )}

            </div>
            <div style={{ gridColumn: 'span 4' }}>
              {total_vehiclerequestLoading ? (
                <UiCard bodyStyle={{ padding: 20 }}>
                  <div style={{ height: 12, width: '40%', background: '#e0e0e0', borderRadius: 4, marginBottom: 12, animation: 'pulse 1.5s ease-in-out infinite' }} />
                  <div style={{ height: 28, width: '60%', background: '#e0e0e0', borderRadius: 4, marginBottom: 8, animation: 'pulse 1.5s ease-in-out infinite' }} />
                  <div style={{ height: 10, width: '50%', background: '#f0f0f0', borderRadius: 4, animation: 'pulse 1.5s ease-in-out infinite' }} />
              ...
              <UiCard bodyStyle={{ padding: 20 }}>
                <div style={{ fontSize: 12, color: '#888', marginBottom: 4 }}>Total VehicleRequest</div>
                <div style={{ fontSize: 28, fontWeight: 700, color: '#e91e63' }}>{(getNestedValue(total_vehiclerequestData ?? {}, 'result.totalCount') as string | number) ?? '—'}</div>
              </UiCard>
              )}
            ...
            <div style={{ gridColumn: 'span 4' }}>
              ...
              <UiCard bodyStyle={{ padding: 20 }}>
                ...
                <div style={{ fontSize: 28, fontWeight: 700, color: '#9c27b0' }}>{(getNestedValue(total_userData ?? {}, 'result.totalCount') as string | number) ?? '—'}</div>
              </UiCard>
              )}
        ...
        <div style={{ gridColumn: 'span 12' }}>
          <h3 style={{ fontSize: 16, fontWeight: 600, margin: '0 0 12px' }}>Recent Department</h3>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(1, 1fr)', gap: 16 }}>
            <div style={{ gridColumn: 'span 1' }}>
              <div style={{ background: '#fff', borderRadius: 8, overflow: 'hidden', border: '1px solid #e8e8e8' }}>
                {recent_department_tableLoading ? (
                  <div style={{ padding: 20, textAlign: 'center', color: '#999' }}>Loading...</div>
                ) : (
                  <table style={{ width: '100%', borderCollapse: 'collapse' }}>
                    <thead><tr><th style={{ padding: '8px 12px', textAlign: 'left', borderBottom: '2px solid #e0e0e0', fontSize: 12, color: '#666' }}>ID</th><th style={{ padding: '8px 12px', textAlign: 'left', borderBottom: '2px solid #e0e0e0', fontSize: 12, color: '#666' }}>Name</th></tr></thead>
                    <tbody>
                      {(Array.isArray(recent_department_tableData) ? recent_department_tableData : extractArray(recent_department_tableData)).map((row: Record<string, unknown>, i: number) => (
                        <tr key={i}><td style={{ padding: '8px 12px', borderBottom: '1px solid #f0f0f0', fontSize: 13 }}>{String(row['id'] ?? '')}</td><td style={{ padding: '8px 12px', borderBottom: '1px solid #f0f0f0', fontSize: 13 }}>{String(row['name'] ?? '')}</td></tr>
                      ))}
                    </tbody>
                  </table>
                )}
        ...
        <div style={{ gridColumn: 'span 12' }}>
          <h3 style={{ fontSize: 16, fontWeight: 600, margin: '0 0 12px' }}>Recent VehicleRequestType</h3>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(1, 1fr)', gap: 16 }}>
            <div style={{ gridColumn: 'span 1' }}>
              <div style={{ background: '#fff', borderRadius: 8, overflow: 'hidden', border: '1px solid #e8e8e8' }}>
                {recent_vehiclerequesttype_tableLoading ? (
                  <div style={{ padding: 20, textAlign: 'center', color: '#999' }}>Loading...</div>
                ) : (
                  <table style={{ width: '100%', borderCollapse: 'collapse' }}>
                    ...
                    <tbody>
                      {(Array.isArray(recent_vehiclerequesttype_tableData) ? recent_vehiclerequesttype_tableData : extractArray(recent_vehiclerequesttype_tableData)).map((row: Record<string, unknown>, i: number) => (
